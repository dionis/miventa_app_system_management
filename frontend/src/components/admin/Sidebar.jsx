      ...
      >
        <div className="px-8 py-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setCollapsed(!collapsed)} className="hidden md:block p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/10 transition-all duration-300">
              {collapsed ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
            </button>
            <button onClick={() => setMobileOpen(false)} className="md:hidden p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/10 transition-all duration-300">
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-4 overflow-y-auto mt-4">
          {menuItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-5 py-4.5 rounded-2xl transition-all duration-300 ${isActive ? 'text-white font-bold shadow-xl shadow-orange-500/10 scale-[1.02]' : 'text-white/60 hover:text-white hover:bg-white/5 font-medium'}`
              }
              style={({ isActive }) => isActive ? { background: 'linear-gradient(135deg, var(--color-brand-gradient-from), var(--color-brand-gradient-to))' } : {}}
            >
        ...
        <div className="px-4 pb-8 mt-auto">
          <div className="flex items-center gap-4 px-5 py-4 rounded-2xl mb-3 border border-white/5" style={{ background: 'rgba(255, 255, 255, 0.03)' }}>
          ...
          <button onClick={handleSignOut} className="flex items-center gap-4 px-5 py-4.5 rounded-2xl w-full text-white/40 hover:text-white hover:bg-white/5 transition-all duration-300 justify-start hover:scale-[1.02]">
